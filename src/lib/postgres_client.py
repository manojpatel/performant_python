        ...
        for attempt in range(max_retries):
            try:
                self._pool = await asyncpg.create_pool(
                    ...
                    command_timeout=60,
                    server_settings={
                        'application_name': 'performant-python'
                    }
                )
...
async def init_postgres(url: Optional[str] = None) -> PostgresPool:
    """Initialize the global PostgreSQL connection pool."""
    global _pg_pool
    if url is None:
        url = os.getenv("POSTGRES_URL", "postgresql://performant:secretpass@localhost:5432/analytics")
