...
async def benchmark_endpoint(name: str, endpoint: str, payload: dict):
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        # Actual test
        tasks = [fetch(client, f"{BASE_URL}{endpoint}", payload) for _ in range(NUM_REQUESTS)]
...
async def main():
    ...
    try:
        res = await benchmark_endpoint(
            "Optimized (Msgspec+Polars)", "/samples/batch-msgspec", payload
        )
        results.append(res)
    except Exception:
        traceback.print_exc()
        print("Msgspec failed")
    ...
    print("\n" + "=" * 80)
    print(
        f"{'IMPLEMENTATION':<30} | {'AVG (ms)':<10} | {'MIN (ms)':<10} | "
        f"{'MAX (ms)':<10} | {'SPEEDUP'}"
    )
    print("=" * 80)
    ...
    for r in results:
        ...
        print(
            f"{r['name']:<30} | {r['avg_ms']:<10.2f} | {r['min_ms']:<10.2f} | "
            f"{r['max_ms']:<10.2f} | {speedup:.2f}x"
        )
    print("=" * 80)
