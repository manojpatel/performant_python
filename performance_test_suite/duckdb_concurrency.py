...
QUERY = (
    "SELECT count(*), avg(val), max(val) FROM large_table WHERE category = (random() * 10)::INTEGER"
)
...
def setup_db() -> None:
    ...
    # Generate ~10M rows. DuckDB is fast, so 10M might be quick,
    # but good enough for parallelism check.
    con.execute("""
