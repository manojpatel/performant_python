...
async def run_iceberg_benchmarks(s3_path: str = "s3://liquid-crystal-bucket-manoj/dumped-clustred-data/source_data_iceberg") -> list[IcebergBenchmarkResult]:
    """
    ...
    queries = [
        ...
        },
        {
            ...
            "sql": f"SELECT COUNT(*) as count FROM iceberg_scan('{optimized_path}') WHERE ARDV = '2025-05-31'"
        },
        {
            "name": "Aggregation by Cluster Key",
            "sql": f"SELECT CAST(ARDV AS VARCHAR) as ARDV, COUNT(*) as count FROM iceberg_scan('{optimized_path}') GROUP BY ARDV"
        },
        {
    ...
    async with pool.connection() as conn:
        ...
        for q in queries:
            try:
                ...
                rows_count = rows[0][0] if rows and len(rows) > 0 and len(rows[0]) > 0 and isinstance(rows[0][0], (int, float)) else len(rows)
                
                results.append(IcebergBenchmarkResult(
                    test_name=q["name"],
                    duration_ms=duration,
                    result_summary={"value": str(rows[0][0]) if rows and len(rows) > 0 and len(rows[0]) > 0 else "Empty"},
