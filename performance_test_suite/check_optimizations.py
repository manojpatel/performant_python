...
async def main():
    ...
    async with pool.connection() as conn:
        ...
        res = conn.execute(f"SELECT COUNT(*) FROM iceberg_scan('{FOLDER_PATH}')").fetchall()
        ...
        query = f"SELECT COUNT(*) FROM iceberg_scan('{METADATA_FILE}') WHERE ARDV = '{PARTITION_FILTER}'"
        t0 = time.perf_counter()
        ...
        res = conn.execute(f"SELECT POS_CD FROM iceberg_scan('{METADATA_FILE}') LIMIT 10").fetchall()
        t1 = time.perf_counter()
        print(f"Duration: {(t1-t0)*1000:.2f}ms")
        ...
        t0 = time.perf_counter()
        res = conn.execute(f"SELECT CAST(ARDV as VARCHAR), COUNT(*) FROM iceberg_scan('{METADATA_FILE}') GROUP BY 1").fetchall()
        t1 = time.perf_counter()
        print(f"Duration: {(t1-t0)*1000:.2f}ms")
